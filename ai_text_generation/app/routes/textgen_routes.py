...
class TextGenResource(MethodView):
    ...
    @text_gen.response(400, ErrSchema)
    @text_gen.response(500, ErrSchema)
    def post(self, data):
        ...
        try:
            ...
            new_text = GeneratedText(user_id=uid, prompt=prompt, response=response)
...
class TextGenByIdResource(MethodView):
    ...
    @text_gen.response(400, ErrSchema)
    @text_gen.response(404, ErrSchema)
    def get(self, id):
        user_id = get_jwt_identity()
        uid = uuid.UUID(user_id)
        text = GeneratedText.query.filter_by(id=id, user_id=uid).first()
        if not text:
            return error_response("text not found", status=404)
        return text

    @jwt_required()
    @text_gen.response(204)
    @text_gen.response(400, ErrSchema)
    @text_gen.response(404, ErrSchema)
    def delete(self, id):
        user_id = get_jwt_identity()
    ...
    def put(self, data, id):
        ...
        try:
            response = ai_service.get_response(prompt)
            text.prompt, text.response = prompt, response
            db.session.commit()
            return text
        except Exception as e:
            return error_response(str(e), status=500)
